...
**Professional judgment over mechanical transcription.** While auditing the Lehrplan against the official source document, the source's own page for the ion product of water carried the label "Ks (und pKs)" — Ks being the symbol for acid dissociation constant, not water's ion product. Mirroring that verbatim would have made one symbol mean two different things in the same chapter, since Ks is correctly used elsewhere in S7.2.3 for actual acid constants. The syllabus text here keeps the chemically correct `Kw` (ion product of water) instead of copying the source's apparent typo — a case where transcribing the source exactly would have been the wrong call.
